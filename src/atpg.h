...
class ATPG {
 public:
  ...
  void diagnosis_fault_insert();
  void diagnosis_map_insert();
  void gen_FailLog();
  int find_f_index(const string &);
  vector<vector<string>> diag_flist;
   void single_diag();

  bool get_diag_diag() { return diag_diag; }
  void read_failLog(const string &);
  void diag_initial_candidate();
  bool diag_remove_candidate();
  void diag_keep_remove_candidate();
 ...
 private:
  ...
  //ssf_diag
  forward_list<fptr> ssf_flist;   /* ssf_diag fault list */
  ...
  bool diag_diag {};
  bool ssf_det = false;
  vector<unordered_map<string, bool>> exact_response; //ssf_diag
  vector<unordered_map<string, bool>> cand_response; //ssf_diag 
  int DFS_times = 0;
  int num_failptn = 0;
  vector<vector<FLOG*>> failLog;
  unordered_map<string, wptr> failPOs;
  unordered_map<fptr, wptr> candidate_faults; /* fault candidates and its corresponding wire*/
  void DFS(const wptr);
  void diag_greedy_ranking();
  void diag_greedy_ranking_2nd_iter(vector<fptr>&);
  void diag_faultsim_by_level(fptr, fptr);
  void diag_print_result(const vector<fptr>&);
  void diag_ssf_print_result(fptr);


  vector<wptr> ssf_cand, temp;
  unordered_map<string, int> wire2index;
  unordered_map<string, int> type2int; 
  void ssf_DFS(const wptr);
  ///
  ...
  ///diag
  void diag_fsim_vectors();
  void diag_fsim_a_vector(const string &vec, int i);
  void ssf_diag_sim_a_vector(const string &vec, fptr, int vecidx); //ssf_diag
  //void diag_fsim_evaluate(const wptr w);
  wptr diag_get_faulty_wire(fptr, int &);
  wptr diag_GI_get_faulty_wire(list<fptr>, int &);
  bool diag_GI_non_control(list<fptr>,int &);

  void diag_fault_sim_evaluate(const wptr );
  ...
  class WIRE {
   public:
    ...
    bool diag_flag;
    int DFS_index = 0;
    forward_list<fptr> flist_w;   /* flist(coppalsed) for this wire */
    void print() { cout<< name << "good value: "<< value<< "faulty value: "<< (wire_value2 & 1) << endl; }
    ///
  ...
  class FAULT {
   public:
    ...
    ///diag
    short diag_flag;              /* diagnosis flag */
    EQUIVALENT_LIST* fault_linklist; /* link list for equivalent faults */
    unsigned greedy_score_single = 0;
    unsigned greedy_score_all = 0;
    bool choosed = 0;
    // bool diag_is_candidate = 0;   /* 1 = this fault is a candidate (not srue if this var is necessary) */
    ///
  }; // class FAULT
  ...
  class FLOG {
   public:
    ...
    int failing_index; /* index for test pattern */
    bool failing_type; /* 0=H/L(expect H, observe L) 1=L/H(expect L, observe H) */
  }; // class FLOG

  class EQUIVALENT_LIST {
   public:

    EQUIVALENT_LIST* next_fault = nullptr;
    EQUIVALENT_LIST* tail = nullptr;
    // bool is_head = false;
    string fault_name;
  }; // class EQUIVALENT_LIST
