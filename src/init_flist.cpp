...
void ATPG::generate_fault_list() {
  ...
  nptr n;
  fptr_s f;
  unordered_map<wptr, pair<int, EQUIVALENT_LIST*>> num_of_eqv_sa0, num_of_eqv_sa1;
  ...
  for (auto pos : sort_wlist) {
    ...
    switch (n->type) {
      ...
      case BUF:
        for (wptr wptr_ele: w->inode.front()->iwire) {
          f->eqv_fault_num += num_of_eqv_sa0[wptr_ele].first;   // count how many equivalent faults in gate input

          EQUIVALENT_LIST* list_to_be_merged;
          if(num_of_eqv_sa0[wptr_ele].second == nullptr){
            //wptr_ele is fanout brach
            list_to_be_merged = new EQUIVALENT_LIST;
            list_to_be_merged->fault_name = wptr_ele->name.substr(0,wptr_ele->name.find("("))
                                           + " " + n->name +" GI SA0";
            list_to_be_merged->tail = list_to_be_merged;
          }
          else{
            list_to_be_merged = num_of_eqv_sa0[wptr_ele].second;
          }
          
          if(f->fault_linklist==nullptr){
            f->fault_linklist = list_to_be_merged;
          }
          else{
            f->fault_linklist->tail->next_fault = list_to_be_merged;
            f->fault_linklist->tail = list_to_be_merged->tail;
          }
        }
        break;
      case NOR:
      case NOT:
        for (wptr wptr_ele: w->inode.front()->iwire) {
          f->eqv_fault_num += num_of_eqv_sa1[wptr_ele].first;

          EQUIVALENT_LIST* list_to_be_merged;
          if(num_of_eqv_sa1[wptr_ele].second == nullptr){
            //wptr_ele is fanout brach
            list_to_be_merged = new EQUIVALENT_LIST;
            list_to_be_merged->fault_name = wptr_ele->name.substr(0,wptr_ele->name.find("("))
                                           + " " + n->name +" GI SA0";
            ...
            list_to_be_merged = num_of_eqv_sa1[wptr_ele].second;
          }
    ...
    if (w->onode.size() > 1 || w->onode.front()->type == EQV || w->onode.front()->type == OR
      ...
      w->flist_w.push_front(f.get());
      flist.push_front(move(f));  // push into the fault list
    } else {
      EQUIVALENT_LIST* new_eq_class = new EQUIVALENT_LIST;
      new_eq_class->fault_name = w->name.substr(0,w->name.find("("))
                                + " " + n->name +" GO SA0";
      new_eq_class->next_fault = f->fault_linklist;
      if(f->fault_linklist==nullptr) new_eq_class->tail = new_eq_class;
      else new_eq_class->tail = f->fault_linklist->tail;
      assert(new_eq_class->tail!=nullptr);
      num_of_eqv_sa0[w] = pair<int, EQUIVALENT_LIST*>(f->eqv_fault_num, new_eq_class);
    }
      ...
      case NOT:
        for (wptr wptr_ele: w->inode.front()->iwire) {
          if (num_of_eqv_sa0.find(wptr_ele) == num_of_eqv_sa0.end())
            cerr << wptr_ele << " is not in hashmap." << endl;
          f->eqv_fault_num += num_of_eqv_sa0[wptr_ele].first;
    ...
    if (w->onode.size() > 1 || w->onode.front()->type == EQV || w->onode.front()->type == AND
      ...
      assert(new_eq_class->tail!=nullptr);
      num_of_eqv_sa1[w] = pair<int, EQUIVALENT_LIST*>(f->eqv_fault_num, new_eq_class);
    }

    /*if w has multiple fanout branches */
    if (w->onode.size() > 1) {
      num_of_eqv_sa0[w] = num_of_eqv_sa1[w] = pair<int, EQUIVALENT_LIST*>(1, nullptr);
  ...
  for (fptr f: flist_undetect) {
    ...
    ////////
    // cerr<<sort_wlist[f->to_swlist]->name.substr(0,sort_wlist[f->to_swlist]->name.find("("))
    //     <<" "<<f->node->name<<" "<<(f->io?"GO":"GI")<<" SA"<<f->fault_type
    //     <<" :eqv_fault_num("<<f->eqv_fault_num<<")"<<endl;
    // EQUIVALENT_LIST* linklist = f->fault_linklist;
    // while(linklist!=nullptr){
    //   cerr<<"  eqv fault:"<<linklist->fault_name<<endl;
